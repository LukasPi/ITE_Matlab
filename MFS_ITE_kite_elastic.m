function  [w_m,sigma_min]=MFS_ITE_kite_elastic(m,init,eps,Rrel_S,mu,lambda,rho)

% ROUTINE: Computes approximate elastic ITEs for kite scatterer via modified MFS
% INPUT:  
%   m = number of kite boundary collocation points (or equivalently source points)
%   init = initial guess for approximate ITE location
%   eps = kite deformation parameter    
...
% Generation of computational points arrays     
    ...
    phi=phi(1:end-1); 
    collocation_x=0.75*cos(phi)+eps*cos(2*phi);
    collocation_y=sin(phi);
    collocation=[collocation_x ; collocation_y];
    % Normal vectors along collocation boundary
    normals_x=cos(phi)./sqrt(cos(phi).^2+(0.75*sin(phi)+2*eps*sin(2*phi)).^2);
    normals_y=(0.75*sin(phi)+2*eps*sin(2*phi))./sqrt(cos(phi).^2+(0.75*sin(phi)+2*eps*sin(2*phi)).^2);
...
% Modified MFS main routine
    ...
    %[w_m,sigma_min]=fminbnd(f_tmp,init-0.05,init+0.05,optimset('TolX',1e-16));
