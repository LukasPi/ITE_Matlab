function  [w_m,sigma_min]=MFS_ITE_polygon_elastic(m,init,p,Rrel_S,mu,lambda,rho)

% ROUTINE: Computes approximate elastic ITEs for regular p_Polygon of unit length with modified MFS
% INPUT:  
%   m = number of polygon boundary collocation points (or equivalently source points)
%   init = initial guess for approximate ITE location
%   p = number of vertices of polygon    
%   Rrel_S = scaling factor for scaling-translated sources wrt polygon
...
% Polygon parametrization
    ...
    % midpoint to vertex
    MV=0.5/sin(pi/p);
    % generate equidistant polygon coordinates
    collocation_x=zeros(1,m);
    collocation_y=zeros(1,m);
    normals_x=zeros(1,m);
    normals_y=zeros(1,m);
    collocation_x(1,1)=ME;
    collocation_y(1,1)=0;
    normals_x(1)=1.;
    normals_y(1)=0.;
    for ii=1:(m-1)
        ...
        collocation_x(1,ii+1)=R*cos(2*pi*ii/m);
        collocation_y(1,ii+1)=R*sin(2*pi*ii/m);
        normals_x(ii+1)=cos(section*2*pi/p);
        normals_y(ii+1)=sin(section*2*pi/p);
    end
...
% Generation of computational points arrays  
    ...
    % Collocation points     
    collocation=[collocation_x ; collocation_y];
    % Source points (scaled version of collocation boundary)
    sources=[Rrel_S*collocation_x ; Rrel_S*collocation_y];
...
% Modified MFS main routine
    ...
    [w_m,sigma_min]=fminsearch(f_tmp,init,optimset('TolX',1e-16));
